...
{
    // didn't konow that string can be written like this
...
static int elf_load_program(struct elf_header *header)
{
    ...
    for (i = 0; i < header->program_header_num; i++) {
        ...
        memcpy((char *)phdr->physical_adddr, (char *)header + phdr->offset, phdr->file_size);
        memcpy((char *)phdr->physical_adddr + phdr->file_size, 0, phdr->memory_size - phdr->file_size);
    }
    return 0;
}
